...
const headerStyles = tv({
  base: 'px-2 h-6 flex items-center gap-1 fixed w-full max-w-full overflow-x-hidden  top-0 z-50',
  variants: {
    theme: {
  ...
  return (
    <>
      <header className={headerStyles({ theme, className })} {...props}>
        {user?.isSubscriber && (
          <FaCrown className="fill-importance5 min-w-3 mr-0.5" size={14} />
        )}
        <div className="w-full flex items-center gap-1 max-sm:w-[90%] overflow-x-hidden">
          {paths.map((path, i) => (
